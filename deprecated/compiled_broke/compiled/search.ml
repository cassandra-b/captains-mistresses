...
let rec greedySearch board eval : board = 
		...
		in let player = (if (ply mod 2 = 0) then P1 else P2)
		in match willLose board player with
			| Some n -> board.(n) <- player; board
			| None ->
...
(* depth-limited negamax *)
	...
	in let player = (if (ply mod 2 = 0) then P1 else P2)
	in match willLose board player with
		| Some n -> (board.(n) <- player; (board, eval board))
		| None ->
		if (depth < 1) then (board, eval board)
			...
			in if (player = P1)
					then maxi board ply depth
				else mini board ply depth
			end
...
let alphabeta depth board eval : (board * float * float) =
(* depth-limited negamax *)
	let ply = whatPly board
	in let player = (if (ply mod 2 = 0) then P1 else P2)
	in match willLose board player with
		| Some n -> (board.(n) <- player; let x = eval board in (board, x, x))
		| None ->
		if (depth < 1) then (let x = eval board in (board, x, x))
		else begin
			let rec mini brd ply dp a b cb : (board * float * float) = 
				begin
				if (dp < 1) then (let x = eval brd in (brd, x, x)) (* max depth, run eval function *)
				else let rec f lst curBestBoard a b : (board * float * float) =
					match lst with
						| hd::[] -> begin
							let bored, alpha, beta = (match maxi hd (ply+1) (dp-1) a b curBestBoard with
									| (b,c,d) -> b, c, d)
							in if (alpha < b)
									then (hd, a, alpha)
								else (curBestBoard, a, b)
									end
						| hd::tl -> begin
							let bored, alpha, beta = (match maxi hd (ply+1) (dp-1) a b curBestBoard with
									| (b,c,d) -> b, c, d)
							in if (alpha < a)
									...
									then f tl hd a alpha
								else f tl curBestBoard a b
									end
						| [] -> (curBestBoard, a, b)
					in f (posMoves P2 brd) cb a b
				end
			and maxi brd ply dp a b cb : (board * float * float) = 
				begin
					if (dp < 1) then (let x = eval brd in (brd, x, x))
					else let rec f lst curBestBoard a b : (board * float * float) =
						match lst with
						| hd::[] -> begin
							let bored, alpha, beta = (match mini hd (ply+1) (dp-1) a b curBestBoard with
									| (b,c,d) -> b, c, d)
							in if (beta > a)
									then (hd, beta, b)
								else (curBestBoard, a, b)
						...
						| [] -> (curBestBoard, a, b)
					in f (posMoves P1 brd) cb a b
				end
			in if (player = P1)
					then (maxi board ply depth (-200.) 200. (newBoard ()))
				else (mini board ply depth (-200.) 200. (newBoard ()))
			end

module AlphabetaLines : Player =
struct
	let toString : string = "Depth-limited Alphabeta search using utilLines heuristic"
	let nextMove board : board = 
		match alphabeta 5 board utilLines with
		| (b,_,_) -> b
end

module AlphabetaDots : Player =
struct
	let toString : string = "Depth-limited Alphabeta search using utilDots heuristic"
	let nextMove board : board = 
		match alphabeta 5 board utilDots with
		| (b,_,_) -> b
end

module AlphabetaHeightLines : Player =
struct
	let toString : string = "Depth-limited Alphabeta search using utilHeightLines heuristic"
	let nextMove board : board = 
		match alphabeta 5 board utilHeightLines with
		| (b,_,_) -> b
end

module AlphabetaHeightDots : Player =
struct
	let toString : string = "Depth-limited Alphabeta search using utilHeightDots heuristic"
	let nextMove board : board = 
		match alphabeta 5 board utilHeightDots with
		| (b,_,_) -> b
end

module AlphabetaWeightLines : Player =
struct
	let toString : string = "Depth-limited Alphabeta search using utilWeightLines heuristic"
	let nextMove board : board = 
		match alphabeta 5 board utilWeightLines with
		| (b,_,_) -> b
end

module AlphabetaWeightDots : Player =
struct
	let toString : string = "Depth-limited Alphabeta search using utilWeightDots heuristic"
	let nextMove board : board = 
		match alphabeta 5 board utilWeightDots with
		| (b,_,_) -> b
end

module AlphabetaHWLines : Player =
struct
	let toString : string = "Depth-limited Alphabeta search using utilHWLines heuristic"
	let nextMove board : board =
		match alphabeta 5 board utilHWLines with
		| (b,_,_) -> b
end

module AlphabetaHWDots : Player =
struct
	let toString : string = "Depth-limited Alphabeta search using utilHWDots heuristic"
	let nextMove board : board = 
		match alphabeta 5 board utilHWDots with
		| (b,_,_) -> b
end

module AlphabetaCowardLines : Player =
struct
	let toString : string = "Depth-limited Alphabeta search using utilCowardLines heuristic"
	let nextMove board : board = 
		match alphabeta 5 board utilCowardLines with
		| (b,_,_) -> b
end

module AlphabetaCowardDots : Player =
struct
	let toString : string = "Depth-limited Alphabeta search using utilCowardDots heuristic"
	let nextMove board : board = 
		match alphabeta 5 board utilCowardDots with
		| (b,_,_) -> b
end
