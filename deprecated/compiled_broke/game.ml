...
let intWinLine (n:int) : int list =
	if (n < 0) 
		then begin print_endline "intWinLine called with n<0"; [] end
	else if (n < 24) (* horizontal *)
		then n::(n+6)::(n+12)::(n+18)::[]
	else if (n < 45) (* vertical *)
		then let i = n-24
		in let x = (i mod 3) + (6 * (i/3))
		in x::(x+1)::(x+2)::(x+3)::[]
		...
		in let x = (i mod 3) + (6 * (i/3)) + 18
		in (x)::(x - 5)::(x - 10)::(x - 15)::[]
	else begin print_endline "intWinLine called with n>68"; [] end	

let linetoBoardIndex (n:int) (i:int) : int =
(* returns the board index, given the line and index in the line *)
	if (n < 0) 
		then begin print_endline "linetoBoardIndex called with n<0"; -1 end
	else if (n < 24) (* horizontal *)
		then match i with
			| 0 -> n
			| 1 -> n+6
			| 2 -> n+12
			| 3 -> n+18
			| _ -> print_endline "linetoBoardIndex called with not -1<i<4"; -1
	else if (n < 45) (* vertical *)
		then let j = n-24
		in let x = (j mod 3) + (6 * (j/3))
		in match i with
			| 0 -> x
			| 1 -> x+1
			| 2 -> x+2
		...
		then let j = n-57 (* (n-57)+18 because you only search the right half *)
		in let x = (j mod 3) + (6 * (j/3)) + 18
		in match i with
			| 0 -> x
			| 1 -> x-5
			| 2 -> x-10
			| 3 -> x-15
			| _ -> print_endline "linetoBoardIndex called with not -1<i<4"; -1
	else begin print_endline "linetoBoardIndex called with n>68"; -1 end	
