...
(* returns the index of the spot needed for them to win or None*)
	let rec sm p x y : int option =
		if (p = player)
			then if ((x mod 6) = 0)
					then Some x
				else if (board.(x-1) = Blank)
						then f (y+1)
					else Some x
		else f (y+1)
	and f n : int option = 
		if (n > 68) then None
			...
			| P1::P1::P1::Blank::[] -> sm P2 (linetoBoardIndex n 3) n
			| P1::P1::Blank::P1::[] -> sm P2 (linetoBoardIndex n 2) n
			| P1::Blank::P1::P1::[] -> sm P2 (linetoBoardIndex n 1) n
			| Blank::P1::P1::P1::[] -> sm P2 (linetoBoardIndex n 0) n
			(**)
			| P2::P2::P2::Blank::[] -> sm P1 (linetoBoardIndex n 3) n
			| P2::P2::Blank::P2::[] -> sm P1 (linetoBoardIndex n 2) n
			| P2::Blank::P2::P2::[] -> sm P1 (linetoBoardIndex n 1) n
			| Blank::P2::P2::P2::[] -> sm P1 (linetoBoardIndex n 0) n
			| _ -> f (n+1)
	in f 0
